...
SlideDeck.prototype.loadConfig_ = function(config) {
  ...
  if (this.config_.presenters) {
    ...
    if (presenters.length == 1) {
      ...
      var gplus = p.gplus ?
        '<a href="' + p.gplus + '">' + p.gplus.replace(/https?:\/\//, '') + '</a>'
        : '';

      var twitter = p.twitter ?
          '<a href="http://twitter.com/' + p.twitter + '">@' + p.twitter + '</a>'
          : '';

      var www = p.www ?
          '<a href="' + p.www + '">' + p.www.replace(/https?:\/\//, '') + '</a>'
          : '';
      ...
      for (var i = 0, p; p = presenters[i]; ++i) {
        var twitterUrl = 'http://twitter.com/' + p.twitter.replace(/@/, '');
        html.push(p.name + ' - ' + '<a href="' + twitterUrl + '">' + p.twitter + '</a>');
      }
      html = html.join('<br>');
      if (dataConfigContact) {
